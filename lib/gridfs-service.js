...
GridFSService.prototype.connect = function (cb) {
  ...
  if (!this.db) {
    ...
    self.options.reconnectTries = 60;
    self.options.reconnectInterval = 1000;
    self.options.useNewUrlParser = true;
    self.options.poolSize = 10;
    self.options.bufferMaxEntries = 0;
    ...
    MongoClient.connect(url, self.options, (error, client) => {
      ...
      console.log('gridfs lib connected to mongo');
